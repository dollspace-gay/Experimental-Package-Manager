...
pub struct Config {
    ...
    /// System root directory (for installing to alternate roots like /mnt/rootfs)
    /// This is NOT serialized - it's set via --root CLI flag only
    #[serde(skip)]
    pub sysroot: PathBuf,
}
...
pub struct OptionsConfig {
    /// Files/directories that should never be extracted from packages.
    /// Supports glob patterns (e.g., "usr/share/info/dir", "usr/share/locale/*").
    /// These files are completely skipped during installation.
    #[serde(default = "default_no_extract")]
    pub no_extract: Vec<String>,

    /// Files that should be preserved during package upgrades.
    /// If a file is modified locally, upgrading won't overwrite it.
    /// Supports glob patterns.
    #[serde(default)]
    pub no_upgrade: Vec<String>,

    /// Files that can be owned by multiple packages without triggering conflicts.
    /// Typically used for shared index files like /usr/share/info/dir.
    /// Supports glob patterns.
    #[serde(default = "default_shared_files")]
    pub shared_files: Vec<String>,

    /// Force overwrite of files owned by other packages.
    /// This is primarily used for bulk installations (like building ISOs)
    /// where packages may have legitimate file overlaps.
    /// NOT serialized - only set via CLI --force flag.
    #[serde(skip)]
    pub force_overwrite: bool,
}

fn default_no_extract() -> Vec<String> {
    vec![
        // The texinfo directory file is dynamically generated by install-info
...
fn default_shared_files() -> Vec<String> {
    vec![
        // These files are legitimately written to by multiple packages
        "usr/share/info/dir".to_string(),
    ]
}
...
impl OptionsConfig {
    /// Check if a path should not be extracted (matches no_extract patterns)
    pub fn should_skip_extract(&self, path: &str) -> bool {
        // Normalize path - remove leading slash if present
        let normalized = path.trim_start_matches('/');
        self.no_extract.iter().any(|pattern| {
            let pattern = pattern.trim_start_matches('/');
            glob_match(pattern, normalized)
        })
    }

    /// Check if a path is a shared file (can be owned by multiple packages)
    pub fn is_shared_file(&self, path: &str) -> bool {
        let normalized = path.trim_start_matches('/');
        self.shared_files.iter().any(|pattern| {
            let pattern = pattern.trim_start_matches('/');
            glob_match(pattern, normalized)
        })
    }

    /// Check if a path should not be upgraded
    pub fn should_preserve(&self, path: &str) -> bool {
        let normalized = path.trim_start_matches('/');
        self.no_upgrade.iter().any(|pattern| {
            let pattern = pattern.trim_start_matches('/');
            glob_match(pattern, normalized)
        })
    }
...
/// Simple glob matching for configuration patterns
/// Supports:
    ...
    if pattern.contains("**") {
        ...
        if parts.len() == 2 {
            ...
            let prefix_matches = if prefix.is_empty() {
                ...
                path.starts_with(prefix) || glob_match_simple(prefix, path.split('/').next().unwrap_or(""))
            };
...
/// Simple glob matching with only * wildcards (no **)
fn glob_match_simple(pattern: &str, text: &str) -> bool {
    ...
    // For single * patterns between prefix and suffix, the above checks are sufficient
    true
}
...
impl Config {
    ...
    /// Set the system root directory (for --root flag)
    ...
    /// Resolve an absolute path relative to sysroot
    /// e.g., if sysroot is /mnt/rootfs and path is /var/lib/rookpkg,
    /// returns /mnt/rootfs/var/lib/rookpkg
    pub fn resolve_path(&self, path: &Path) -> PathBuf {
        if self.sysroot == Path::new("/") {
            path.to_path_buf()
        } else {
            // Strip leading slash from path and join with sysroot
            let stripped = path.strip_prefix("/").unwrap_or(path);
            self.sysroot.join(stripped)
        }
    }

    /// Get the database path, resolved relative to sysroot
    pub fn database_path(&self) -> PathBuf {
        self.resolve_path(&self.database.path)
    }

    /// Get the hooks directory, resolved relative to sysroot
    pub fn hooks_dir(&self) -> PathBuf {
        self.resolve_path(&self.hooks.hooks_dir)
    }
}
