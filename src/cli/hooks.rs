...
pub fn list(config: &Config) -> Result<()> {
    ...
    // Use custom hooks dir from config if set (resolved relative to sysroot)
    if hooks_dir != config.resolve_path(Path::new("/etc/rookpkg/hooks.d")) {
        manager = HookManager::with_hooks_dir(root, &hooks_dir);
    }
...
pub fn install(hook_path: &Path, order: Option<u32>, config: &Config) -> Result<()> {
    ...
    let root = config.sysroot();
    let hooks_dir = config.hooks_dir();
    let manager = if hooks_dir != config.resolve_path(Path::new("/etc/rookpkg/hooks.d")) {
        HookManager::with_hooks_dir(root, &hooks_dir)
    } else {
