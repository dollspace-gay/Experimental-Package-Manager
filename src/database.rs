...
impl Database {
    ...
    fn run_migrations(&self) -> Result<()> {
        // Check if files table exists and has old schema (UNIQUE on path alone)
        // We detect this by checking if the unique index on just 'path' exists
        let has_old_unique: bool = self.conn.query_row(
            r#"
            SELECT COUNT(*) > 0
            FROM sqlite_master
            WHERE type = 'index'
        ...
        if has_old_unique {
            tracing::info!("Migrating files table to support shared ownership...");

            // SQLite doesn't support DROP CONSTRAINT, so we need to recreate the table
            self.conn.execute_batch(
                r#"
                -- Create new files table with updated constraint
            ...
            tracing::info!("Migration complete - files table now supports shared ownership");
        }
    ...
    fn initialize(&self) -> Result<()> {
        ...
        self.conn.execute_batch(
            ...
            -- RPM-style: multiple packages can own the same file path
            -- The (package_id, path) combo must be unique (same package can't list same file twice)
    ...
    /// Check if a file path is owned by any package (returns first owner for backwards compat)
    pub fn file_owner(&self, path: &str) -> Result<Option<String>> {
        let owners = self.file_owners(path)?;
        Ok(owners.into_iter().next())
    }

    /// Get all packages that own a file path (RPM-style shared ownership)
    pub fn file_owners(&self, path: &str) -> Result<Vec<String>> {
        let mut stmt = self.conn.prepare(
            r#"
            SELECT p.name
            FROM files f
        ...
        rows.collect::<Result<Vec<_>, _>>()
            .context("Failed to get file owners")
    }
